...
module.exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    res.status(200).json(success(users, {count: users.length}));
  } catch (err) {
    next(err);
  }
};
...
module.exports.updateUser = async ({params : {id}, body}, res, next) => {
    try {
 ...
 * @access   Private
 */
module.exports.deleteUser = async ({params : {id}}, res, next) => {
    try {
...
module.exports.restoreUser = async ({params : {id}}, res, next) => {
    try {
